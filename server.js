...
async function api(req,res,url){
 ...
 if(url.pathname.startsWith('/api/admin/')){
  ...
  if(req.method==='POST'&&resource==='freight-quote'){
   ...
   const routes=await googleJson('https://routes.googleapis.com/directions/v2:computeRoutes',{method:'POST',headers:{'Content-Type':'application/json','X-Goog-Api-Key':GOOGLE_MAPS_API_KEY,'X-Goog-FieldMask':'routes.distanceMeters,routes.duration,routes.polyline.encodedPolyline,routes.travelAdvisory.tollInfo,routes.legs.travelAdvisory.tollInfo'},body:JSON.stringify({origin:{address:origin},destination:{address:destination},travelMode:'DRIVE',routingPreference:'TRAFFIC_UNAWARE',computeAlternativeRoutes:false,languageCode:'pt-BR',regionCode:'br',units:'METRIC',extraComputations:['TOLLS'],routeModifiers:{vehicleInfo:{emissionType:'DIESEL'}}})}),route=routes.routes?.[0];
  ...
  if(req.method==='PUT'&&resource==='vehicles'){
   const d=await readBody(req),targetId=Number(url.searchParams.get('id')),plate=String(d.plate||'').replace(/[^a-zA-Z0-9]/g,'').toUpperCase(),renavam=digits(d.renavam),fuelConsumption=Number(d.fuelConsumption);if(!targetId||plate.length!==7||!d.model||!d.chassis||renavam.length<9||renavam.length>11||!(fuelConsumption>0))return send(res,400,{error:'Preencha os dados e confira placa, Renavam e consumo de combustível.'});
   const result=await pool.query('UPDATE vehicles SET plate=$1,model=$2,chassis=$3,renavam=$4,fuel_consumption=$5 WHERE id=$6 AND company_id=$7',[plate,d.model.trim(),d.chassis.trim().toUpperCase(),renavam,fuelConsumption,targetId,user.company_id]);if(result.rowCount)await pool.query('UPDATE trips SET vehicle=$1 WHERE vehicle_id=$2 AND company_id=$3',[plate,targetId,user.company_id]);return result.rowCount?send(res,200,{ok:true}):send(res,404,{error:'Veículo não encontrado.'});
  }
  ...
  if(req.method==='POST'&&resource==='vehicles'){
   const d=await readBody(req),plate=String(d.plate||'').replace(/[^a-zA-Z0-9]/g,'').toUpperCase(),renavam=digits(d.renavam),fuelConsumption=Number(d.fuelConsumption);if(plate.length!==7||!d.model||!d.chassis||renavam.length<9||renavam.length>11||!(fuelConsumption>0))return send(res,400,{error:'Preencha os dados e confira placa, Renavam e consumo de combustível.'});
   await pool.query('INSERT INTO vehicles (plate,model,chassis,renavam,fuel_consumption,company_id) VALUES ($1,$2,$3,$4,$5,$6)',[plate,d.model.trim(),d.chassis.trim().toUpperCase(),renavam,fuelConsumption,user.company_id]);return send(res,201,{ok:true});
  }
